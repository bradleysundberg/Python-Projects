...
from .forms import AccountForm, TransactionForm, AccountChoiceForm
from .models import Account, Transaction


# This function will render the Home page when requested
def index(request):
    form = AccountChoiceForm(data=request.POST or None)  # Retrieve Transaction form
...
def transaction(request):
    form = TransactionForm(request.POST or None)  # Retrieve the Transaction form
    # Checks if request method is POST
    if request.method == "POST":
        if form.is_valid():  # Check to see if the submitted form is valid and if so, saves the form
